...
Firebase Realtime Database를 기반으로 대(태그) → 중(메인 테스크) → 소(서브 테스크) 구조의 할 일 관리를 돕는 안드로이드 앱입니다. 태그를 중심으로 업무를 분류하고, 각 메인 테스크/서브 테스크의 일자·기간·색상·중요도를 세분화해서 기록하도록 설계했습니다.
...
- 메인 테스크 컬러·태그·일자·기간 입력 및 목록/상세 뷰 표현
...
5. **입력 검증 고도화**: 태그 중복 이름 방지, 기간 포맷 안내, 날짜 유효성 (시작/종료) 추가
